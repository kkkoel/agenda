...
public:
    ...
    std::string final() {
        if (!m_finalized) {
            ...
            for (int i = 0; i < 8; i++) {
                lenBuf[7 - i] = (m_totalLen >> (i * 8)) & 0xFF;
            }
            for (int i = 0; i < 8; i++) {
...
static bool findUser(const std::string& username, std::string& outHash) {
...
bool registerUser(const std::string& username, const std::string& password) {
    if (username.empty() || password.empty()) {
        std::cerr << "[ERROR] Username or password cannot be empty\n";
        return false;
    }
    if (username.find(':') != std::string::npos) {
        std::cerr << "[ERROR] Username cannot contain colon ':'\n";
        return false;
    }
    ...
    if (!fout.is_open()) {
        std::cerr << "[ERROR] Cannot open " << USER_FILE << " for writing\n";
        return false;
    }
...
bool loginUser(const std::string& username, const std::string& password) {
    ...
    if (!findUser(username, storedHash)) {
        std::cerr << "[LOGIN FAILED] User \"" << username << "\" does not exist\n";
        return false;
    }
